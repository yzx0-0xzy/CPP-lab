#include <iostream>
#include <string>
#include <algorithm>//使用remove方法
#include <Windows.h>//修改字体颜色
...
//从main方法开始
int main() {
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_GREEN);//设置字体颜色为绿色
	cout << "<--高精度计算器-->\n";
	cout << "（输入任意字母即可退出）\n";
	while (1) {
		if (!GetNum()) break;
		NumInfo sum = Operator();
		PrintSum(sum);
	}
	cout << "程序已正常退出\n";
	return 0;
...
//对字符串进行小数点移位（两个字符串小数补零对齐），并将移位保存，将字符串小数点,逗号和前缀0全部删除
void RadixPoint(string & str1, string & str2) {
...
//建立双向动态链表保存待计算数据
void BuildList(string & num, NumInfo & numinfo) {
	if (num[0] == '-')
...
NumInfo Operator() {
	...
	if (op == '/') {
		...
		if (SecNum.len == 1 && SecNum.head->num == 0) {
			cout << "除数不能为零！\n";
			Node *p = new Node;
			p->num = 0;
			p->next = p->last = NULL;
...
//加法运算
NumInfo OpPlus(NumInfo fir, NumInfo sec, int NoDelete) {
...
//减法运算
NumInfo OpReduce(NumInfo fir, NumInfo sec, int NoDelete) {
	NumInfo sum;
	//sum.sign = 0;//结果符号默认为正
...
//乘法运算
NumInfo OpMulti(NumInfo fir, NumInfo sec) {
	NumInfo sum;
...
//除法运算
NumInfo OpDivision(NumInfo fir, NumInfo sec) {
	NumInfo sum;
...
//乘数为个位数的算法
NumInfo OpMulti_singel(NumInfo fir, int n) {
	NumInfo sum;
	int carry = 0;//用于保存进位
	Node *p = NULL;//用于保存链表中上一个数据
	if (n == 0) {//遇到乘数为零时直接返回0
...
//输出结果
void PrintSum(NumInfo & num) {
	cout << "计算结果：";
	if (num.sign == 1) cout << "-";//先输出符号
	...
	for (int i = 0; i < num.len; ++i) {
		...
		front = false;
		if (i == num.len - pointMove) cout << ".";
		if (pointMove == 0 && i % 3 == num.len % 3 && i) cout << ",";
		cout << num.tail->num;
		p = num.tail;
		num.tail = num.tail->last;
